...
function normalizeExternalDashboardPayload(data, assignmentSummaries = new Map(), context = null) {
  ...
  const gisUsers = filteredGisUsers.map((user) => {
        ...
        const normalizedUser = {
          ...
          checkinEnabled: optionalBooleanValue(user.checkinEnabled, user.checkin_enabled, user.checkins?.enabled, user.checkins?.is_active) ?? false,
          checkinFrequency: firstValue(user.checkinFrequency, user.checkin_frequency, user.checkins?.frequency_days, user.checkins?.frequency) ?? null,
          checkinPreferredTime: firstValue(user.checkinPreferredTime, user.checkin_preferred_time, user.checkins?.preferred_time, user.checkins?.preferredTime) ?? null,
          checkinLastStatus: firstValue(user.checkinLastStatus, user.checkin_last_status, user.checkins?.last_status, user.checkins?.lastStatus, user.checkins?.last_outcome, user.checkins?.lastOutcome) ?? null,
          checkinLastReportedAt: firstValue(user.checkinLastReportedAt, user.checkin_last_reported_at, user.checkins?.last_reported_at, user.checkins?.lastReportedAt, user.checkins?.last_checkin_at, user.checkins?.lastCheckinAt) ?? null,
          careProviderNames,
