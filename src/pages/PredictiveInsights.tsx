...
function demoHorizon(days: number): HorizonRow[] {
  return Array.from({ length: days }, (_, index) => {
    ...
    const urgent = index < 4 ? 2 + (index % 2) : 3 + (index % 3 === 0 ? 1 : 0);
    const review = 6 + Math.round(index / 3);
    const medication = 3 + (index % 3);
    const noResponse = 2 + (index % 2);
    return {
...
function previewPayload(days: number, filter: string): InsightsPayload {
  const clients = filterDemoClients(filter);
  return {
    kpis: {
...
function RiskTrajectory({ client }: { client: InsightClient }) {
  ...
  const padX = 10;
  const padY = 12;
  const allValues = [...history, ...mid, ...low, ...high];
  const minValue = Math.max(0, Math.min(...allValues) - 8);
  const maxValue = Math.min(100, Math.max(...allValues) + 8);
                ...
                return (
                  <div key={row.date} className="rounded-lg border border-border bg-muted/30 p-3">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">{formatShortDate(row.date)}</p>
                      <Badge
                        variant="outline"
                        className={cn(
